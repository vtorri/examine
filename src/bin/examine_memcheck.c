...
void
examine_memcheck_run(const char *module, const char *args)
{
    Exm *exm;
    ...
    if (!exm_file_check(exm, module))
        goto del_exm;

    EXM_LOG_INFO("Command: %s %s", module, args);

    if (!exm_file_map(exm))
    {
        EXM_LOG_ERR("impossible to map filename %s", module);
        goto del_exm;
    }
